<%@ Page Title="" Language="C#" MasterPageFile="~/site.Master" EnableEventValidation="false" AutoEventWireup="true" EnableViewState="false" CodeBehind="ilan-detay.aspx.cs" Inherits="PL.ilan_detaylari" %>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12 page-content col-thin-right">
                    <div class=" text-center" id="specialplace" runat="server">
                    </div>
                </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 automobile-left-col">
                                    ...
                                    <div class="inner">
                                        ...
                                        <%--<div class="key-features" id="detailprop">
                                        </div>--%>
                                    </div>
                                </div>
                                <div class="col-md-6 automobile-left-col">
                            ...
                            <div class="col-lg-12">
                                <div class="no-margin item-carousel owl-carousel owl-theme" style="padding-top: 25px;">
                                    <asp:Repeater ID="rpNearPosition" runat="server">
                                        <ItemTemplate>
                                            <div class="item">
                                                <a href='<%# String.Format("/ilan/{0}-{1}/detay", BLL.PublicHelper.Tools.URLConverter(((BLL.ExternalClass.ilanDataType)Container.DataItem).baslik),((BLL.ExternalClass.ilanDataType)Container.DataItem).ilanId) %>'>
...
<asp:Content ID="Content1" ContentPlaceHolderID="scripts" runat="server">
    ...
    <script type="text/javascript">
        ...
        function getDetail(adsid, opt) {
            var string = "";
            jQuery.ajax({
                type: "POST",
                url: "/services/ki_operation.asmx/getDetailAds",
                dataType: "json",
                data: "{ inadsid:'" + adsid + "'" + ", opt:'" + opt + "' }",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
                    var d = JSON.parse(data.d);
                    if (opt == 1) {
                        for (var i = 0; i < d.length; i++) {
                            var appnd = "<div class='media'><div class='media-body'><span class='data-type'>" + d[i].ozellikAd.toUpperCase() + "</span><span class='media-heading'>" + d[i].deger + "</span></div></div>";
                            $("#detailprop").append(appnd);
                        }
                    }
                    else if (opt == 2) {
                        var appndslider = "";
                        ...
                        if (d.length != 0) {
                            for (var i = 0; i < d.length; i++) {
                                appndslider = "<li><img src='upload/ilan/" + d[i].resim + "' alt='img' width='450' height='300'/></li>";
                                appndpager = "<a class='thumb-item-link' data-slide-index='" + i + "' href='#'><img src='upload/ilan/" + d[i].resim + "' alt='img'/></a>";
                            }
                            $("#bxsliderapp").append(appndslider);
                            $("#bx-pager").append(appndpager);
                        }
                        else {
                            appndslider = "<li><img src='upload/ilan/not-found-image.jpg' alt='img' width='450' height='300'/></li>";
                            $("#bxsliderapp").append(appndslider);
                        }
