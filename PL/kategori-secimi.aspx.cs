...
namespace PL
{
    public partial class kategori_secimi : System.Web.UI.Page
    {
        ...
        protected void devam_Click1(object sender, EventArgs e)
        {
            ...
            DAL.magazaKullanici magazaKullanici = _magazaKullaniciManager.Get(_kullanici.kullaniciId);

            Session["cat"] = null;
            Session["tur"] = null;
            Session["mgz"] = null;

            if (magazaKullanici != null)
            {
                if (magazaKullanici.magaza.pasifMi == false && magazaKullanici.magaza.silindiMi == false & magazaKullanici.magaza.magazaKategori.kategoriId.ToString() == ListBox1.SelectedValue)
                {
                    int _value = magazaKullanici.magaza.magazaId;
